...
// grades share one axis and connect into a single continuous trajectory.
const Y_MIN = 60;
const Y_MAX = 100;
const W = 300;
const H = 150;
...
export function LineTrend({ name, points }: { name: string; points: RawPoint[] }) {
  ...
  const may = series.filter((p) => p.may != null) as { grade: number; may: number }[];
  const dec = series.filter((p) => p.dec != null) as { grade: number; dec: number }[];
  ...
  return (
    <figure className="break-avoid rounded-3xl border border-black/5 bg-white p-4 shadow-sm">
      <figcaption className="mb-1 flex items-baseline justify-between gap-2">
        ...
        {latest != null && (
          <span className="text-sm font-semibold text-brand">
            {latest}% <span className="text-ink/40">cuối L4</span>
          </span>
        )}
      </figcaption>
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" role="img" aria-label={`Xu hướng ${name}`}>
        ...
        {dec.length > 1 && (
          <polyline
            points={line(dec.map((p) => ({ grade: p.grade, v: p.dec })))}
            fill="none"
        ...
        {/* MAY series (solid, brand), continuous across all grades */}
        {may.length > 1 && (
          <polyline
            points={line(may.map((p) => ({ grade: p.grade, v: p.may })))}
            fill="none"
